...
export class EnvoiPaiementSodeciComponent implements OnInit {
                  ...
                  .subscribe(
                    (T24: any) => {
                      ...
                      if (this.reponseT24.statut === 0) {
                        ...
                        this.sodeciTransaction.TypeCanal =
                          this.paramCommissions.typeCanal;

                        this.transactionService
                          .TransactionSodeci(this.sodeciTransaction)
  ...
  getFactureIdAndPer() {
    ...
    this.factureService.CieFactureByIdAndPer(idabon, perfact).subscribe(
      (data) => {
        if (data) {
          let body = {
            idabon: data.idabon,
            perfact: data.perfact,
            statut: 'V',
          };
          this.httpClient
            .put<any>(
              environment.urlFinal + 'efacture/em/CieUpdateStatut',
              body
            )
            .subscribe(
              (newFacture) => {},
